# 1. Pendahuluan
## 1.1. Cakupan materi
- Fokus utamanya hanya pada perintah terminal yang biasa digunakan.
...
- Hanya berlaku untuk Linux/Unix 
...
command     | description
--          | --
```man```   | menampilkan halaman manual. misalnya ```man date```
```date```  | menampilkan waktu dan tanggal
```cal```   | menampilkan kalender
```ls```    | melihat isi suatu directory
```cat```   | menampilkan isi dari suatu file
```cp```    | copy file maupun directory
```mv```    | memindah atau mengubah nama file maupun directory
```diff```  | daftar perbedaan diantara 2 file
```rm```    | hapus file 
```grep```  | menemukan karakter sesuatu kata yang dicari
```pwd```   | print the present working directory's name
```cd```    | mengubah current directory 
...
Alt+F             | memindahkan cursor maju satu kata
Ctrl+F            | memindahkan cursor maju satu karakter
Alt+B             | memindahkan cursor mundur satu kata
Ctrl+B            | memindahkan cursor mundur satu karakter
Ctrl+XX           | menempatkan posisi kursor diantara posisi sekarang dan posisi sebelumnya
 
### 2.2.2. Editing
keyboard shortcut | description
---               |---
Ctrl+U            | hapus seluruh karakter di kiri cursor
Ctrl+K            | hapus seluruh karakter di kanan cursor
Ctrl+W            | hapus satu kata di kiri cursor
Ctrl+H            | hapus satu karakter di kiri cursor
Alt+D             | hapus satu kata di kanan cursor
Ctrl+D            | hapus satu karakter di kanan cursor
Ctrl+Y            | paste karakter yang di cut
Ctrl+_            | undo hapus terakhir
Tab               | autocomplete
...
Ctrl+G            | abort
Ctrl+P/UP         | command sebelumnya in the history
Ctrl+N/DOWN       | command selanjutnya in the history
...
## 16.1. Example
- print sum of 2 numbers<br>```echo $((1+3))```
...
- assign contents of file to a variable<br>```filedata=$(cat file.txt)```<br>```filedata=$(<file.txt)```<br>*second option is faster*
